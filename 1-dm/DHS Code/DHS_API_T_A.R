...
boil_bleach <- merge(boil, bleach, by = c("Country", "Community Type"), all = TRUE)
bleach_filter <- merge(filter, boil_bleach, by = c("Country", "Community Type"), all = TRUE)
...
all_indicators_T <- merge(femaleunderover, treatswater, by = c("Country", "Community Type"), all = TRUE) %>%
  relocate("Country", "Community Type","Treats water with acceptable method", "Boil", "Bleach", "Filter", "SolarDisinfection", "Strain", "More than 30 minutes")
