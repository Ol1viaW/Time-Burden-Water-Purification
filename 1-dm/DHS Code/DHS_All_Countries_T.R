...
for (i in countryIds) {
  ...
  if((ncol(i_PR_w) == 9)) {
    ...
    for (var in variables) {
      ...
      mysurvey <- svydesign(id = ~PSU, data = dta,
                            weight = ~wt, nest = TRUE)
      ...
      if (var == "Boil") {
        weighted_freqs <- svytable(~Boil, mysurvey) 
      }
      else {
        weighted_freqs <- svytable(as.formula(paste0("~", var)), mysurvey)
      }
