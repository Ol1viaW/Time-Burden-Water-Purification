library(readr)
DHS <- read_csv("Data:Results/DHS_MICS_Oct_6.csv")
DHS <- DHS[!(DHS$Type %in% c("MICS")), ]
DHS_MICS <- bind_rows(DHS, MICS)

DHS_MICS <- DHS_MICS %>%
  arrange(country)

write_xlsx(DHS_MICS, path = "DHS_MICS_March_3.csv") 
...
library(dplyr)
library(stringr)
hh_sizes <- read_excel("~/Downloads/Fixed_HH_Sizes.xlsx")
...
merged_data <- merged_data %>%
  select(-"Mean Household Size")

write.csv(merged_data, paste0(here::here(), "/1-dm/DHS_MICS_March_3_Results.csv"))
